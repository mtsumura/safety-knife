from safety_knife.config import ticker_symbol, DATA_DIR, output_path, company_output_path
from safety_knife.bronze.fetch_data import retrieve_daily_data
from safety_knife.bronze.store_data import load_to_spark, save_to_delta, load_from_delta, get_or_create_spark_session
...
def populate_yfinance_company_delta(spark, info):
    ...
    save_to_delta(spark_df_info, company_output_path, spark=spark)
...
# spark_df = prepare_data(spark_df)
# spark_df.show()
# spark_df.printSchema()

# save_to_delta(spark_df, path=output_path, partition="Symbol", spark=spark)

# df0 = load_from_delta(output_path, spark=spark)
# df0.show()
